...
The pipeline reads ROOT-format input files with `uproot` only, clusters jets through FastJet/fjcontrib exposed by hepyy, selects Lund prongs, caches intermediate analysis products, and writes parquet outputs for EEC/CAB analysis.
...
JEWEL HepMC event generation lives in `hepyy-utils`, not in the CAB
...
Install/update the utility package through hepyy once the recipe is
available:
...
The Python backend loads FastJet/fjcontrib with `hepyy.load("fastjet")` and
`hepyy.load("fjcontrib")`. The shell modulefiles advertise the installed
...
FastJet/fjcontrib in memory through `hepyy-utils`, then reuse the CAB
...
is a generated-event demonstration based on the `hepyy-utils` Pythia/FastJet
example. It uses `PythiaConfig.pp_hard_qcd`, direct hepyy loading of
...
cache mechanics come from `hepyy_utils.cache.ArtifactCache`; CAB still owns
...
jets)`. It uses reusable flavor-tagging helpers from `hepyy-utils` and
