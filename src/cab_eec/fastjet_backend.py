...
- Use the `henv` environment and hepyy-style FastJet/fjcontrib access for jet finding and Lund declustering.
...
Build the CAB analysis package in this repository so it reads ROOT-format JEWEL track files only through `uproot`, clusters jets with FastJet via hepyy-style imports, selects prongs from Lund declustering, and writes reusable cached analysis products plus final parquet tables and plots.
...
- Use hepyy-style FastJet setup:
  - `import hepyy; hepyy.load("fastjet"); hepyy.load("fjcontrib")`
...
- Cached splitting records may be larger than final EEC tables, but this is the right tradeoff because it avoids rerunning uproot I/O, jet finding, and Lund declustering when changing EEC binning or normalization.
